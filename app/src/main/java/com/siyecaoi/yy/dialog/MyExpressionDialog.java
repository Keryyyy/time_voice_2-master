...
//        getCall();
    ...
    private void initEmoji() {
        ...
        //如果没有权限移除小黄人表情
        Integer passPort = (Integer) SharedPreferenceUtils.get(MyApplication.getInstance(), Const.User.HAVE_NUMBER_PORT, 0);
        if (passPort != 2){
            EmojiList.DataBean bean = null;
            for (int j=0; j<=emojiListBean.size()-1;j++){
                if (emojiListBean.get(j).getUnicode() == 128630){
                    bean = emojiListBean.get(j);
                }
            }
            if (bean != null){
                emojiListBean.remove(bean);
            }
        }
