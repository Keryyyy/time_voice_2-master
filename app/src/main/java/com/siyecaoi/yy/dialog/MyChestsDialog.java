                        ...
                        try {
                            parser.parse(new URL(imgShow), new SVGAParser.ParseCompletion() {
                                ...
                                public void onComplete(SVGAVideoEntity svgaVideoEntity) {
                                    ...
                                    mSVGAImageViewChests.setImageDrawable(drawable);
                                    mSVGAImageViewChests.startAnimation();
                                }
...
//    }
    ...
    private void setData(List<ChestOpenBean.DataBean.LotteryBean> dataBean) {
        ...
        if (size == 1) {
            if (dataBean.get(0).getImg().endsWith(".svga")) {
                mSVGAImageViewChests.setVisibility(View.VISIBLE);
                ivShowChests.setVisibility(View.INVISIBLE);
                SVGAParser parser = new SVGAParser(context);
                try {
                    parser.parse(new URL(dataBean.get(0).getImg()), new SVGAParser.ParseCompletion() {
                        @Override
                        public void onComplete(SVGAVideoEntity svgaVideoEntity) {
                            SVGADrawable drawable = new SVGADrawable(svgaVideoEntity);
                            mSVGAImageViewChests.setImageDrawable(drawable);
