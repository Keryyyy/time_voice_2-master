        ...
        HttpManager.getInstance().post(Api.UserGold, map, new MyObserver(this) {
            ...
            public void success(String responseString) {
                ...
                if (getOneBean.getCode() == Api.SUCCESS) {
                    ...
                    tvNextGrade.setText("距离下一级还需要消耗" + getOneBean.getData().getGradeNum() + "星球币");
                } else {
...
//        tvGradeGrade.setTypeface(typeface);
        ...
        if (grade > 0) {
            ...
            tvOne.setTextColor(ContextCompat.getColor(getContext(), R.color.white));
            tvOneGrade.setTextColor(Color.parseColor("#81F6B4"));
//            tvOneGrade.setBackgroundResource(R.drawable.bg_round5_ffo1);
        }
        if (grade > 9) {
            ivTwoGrade.setImageResource(R.drawable.privilege_effect1);
            tvTwo.setTextColor(ContextCompat.getColor(getContext(), R.color.white));
            tvOneGrade.setTextColor(Color.parseColor("#46C4FF"));
